...
export default {
    ...
    actions: {
        ...
        //添加动态属性
        async addbutes({dispatch},{id,attr_name,attr_sel,type, pagenum, pagesize }){
            let res = await api.addbutes({
        ...
        //编辑提交参数
        async addchangebutes({dispatch},{id,attrId,attr_name,attr_sel,attr_vals,type, pagenum, pagesize}){
            let res = await api.addchangebutes({
        ...
        //删除参数
        async deletbute({dispatch},{id,attrId,type, pagenum, pagesize}){
            let res = await api.deletbute({
                id,attrId
            })
            if(res.meta.status === 200){
                Message.success(res.meta.msg)
