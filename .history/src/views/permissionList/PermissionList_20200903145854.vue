            ...
            <el-table-column prop="level" label="权限等级" align="center">
              <template slot-scope="scope">
                <el-tag
                  ...
                  :type="scope.row.tag === '1级' ? 'primary' : 'success'"
                  disable-transitions
                >{{scope.row.pay_statuss}}</el-tag>
                <el-tag
                  v-if="scope.row.pay_statuss == '1'"
                  :type="scope.row.tag === '2级' ? 'primary' : 'danger'"
                  disable-transitions
                >{{scope.row.pay_statuss}}</el-tag>
                <el-tag
                  v-if="scope.row.pay_statuss == '2'"
                  :type="scope.row.tag === '3级' ? 'primary' : 'danger'"
                  disable-transitions
                >{{scope.row.pay_statuss}}</el-tag>
              </template>
            </el-table-column>
