  ...
  <div class="UserListbox">
    <div class="userListbox">
      ...
      <div class="userListbox-nav">
        <!-- 用户表格 -->
        <div class="list_b">
          <el-table :data="roles" border stripe style="width: 100%" @expand-change="openexpand">
            <!-- 展开符 -->
            <el-table-column type="expand">
              <template slot-scope>
                <div class="cloumnbox">
                  <div class="cloumnbox1">
                    <div class="cloumnbox2" v-for="item in arrlist" :key="item.id">
                      <!-- 一级权限 -->
                      <div class="cloumnbox2-item" v-if="item.children"><el-tag>{{item.roleName}}</el-tag></div>
                      <!-- 二级权限 -->
                      <div v-if="item.children">
                        <div v-for="item1 in item.children" :key="item1.id">
                        <div class="cloumnbox2-item" v-if="item1.children"><el-tag>{{item1.authName}}</el-tag></div>
                      </div>
                      </div>
                      <!-- 三级权限 -->
                      <div v-if="item1.children">
                        <div v-for="item2 in item1.children" :key="item2.id">
                          <div class="cloumnbox2-item" v-if="item2.children"><el-tag>{{item2.authName}}</el-tag></div>
                        </div>
                      </div>
                    </div>
                  </div>
...
export default {
  ...
  methods: {
    ...
    openexpand(row,expandedRows){
     
       this.arrlist = JSON.parse(JSON.stringify(expandedRows[0]));
